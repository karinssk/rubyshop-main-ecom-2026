...
class PaymentTable extends TableAbstract
{
    public function setup(): void
    {
        $this
            ...
            ->addHeaderAction(
                HeaderAction::make('delete-all')
                    ->label(trans('plugins/payment::payment.delete_all_transactions'))
                    ->icon('ti ti-trash')
                    ->color('danger')
                    ->url(route('payment.destroy-all'))
                    ->permission('payment.destroy')
                    ->withDefaultAction(false)
                    ->addAttribute('class', 'delete-all-transactions-button')
            );
    }
