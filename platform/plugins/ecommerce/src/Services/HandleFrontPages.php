...
class HandleFrontPages
{
    ...
    public function handle(Slug|array $slug): array|Slug
    {
        ...
        switch ($slug->reference_type) {
            ...
            case ProductCategory::class:
                ...
                // Add category schema
                $this->addCategorySchema($category, $products);
    ...
    protected function addCategorySchema(ProductCategory $category, $products): void
    {
        // Build breadcrumb for category schema
        $breadcrumbItems = [];
        $breadcrumbItems[] = [
            '@type' => 'ListItem',
            'position' => 1,
            'name' => __('หน้าแรก'),
            'item' => \Botble\Base\Facades\BaseHelper::getHomepageUrl()
        ];
        ...
        $itemListElements = [];
        $position = 1;
        foreach ($products->take(10) as $product) { // Limit to first 10 products for schema
            $itemListElements[] = [
                '@type' => 'ListItem',
                'position' => $position++,
                'url' => $product->url
        ...
        $schema = [
            ...
            '@graph' => [
                ...
                [
                    '@type' => 'CollectionPage',
                    'name' => 'หมวดหมู่: ' . $category->name,
                    'description' => $category->description ?: 'รวม' . $category->name . 'คุณภาพสูง',
                    'url' => $category->url
                ],
                [
                    '@type' => 'ItemList',
                    'name' => 'รายการสินค้าในหมวด ' . $category->name,
        ...
        Theme::asset()
            ->container('header')
            ->writeScript('category-schema', $schema, attributes: ['type' => 'application/ld+json']);
    }
}
