            ...
            add_action(BASE_ACTION_PUBLIC_RENDER_SINGLE, function ($screen, $object): void {
                add_filter(THEME_FRONT_HEADER, function (?string $html) use ($object) {
                    ...
                    $breadcrumbItems[] = [
                        ...
                        'position' => 1,
                        'item' => [
                            '@id' => BaseHelper::getHomepageUrl(),
                            'name' => __('หน้าแรก')
                        ]
                    ];

                    $category = $object->categories->sortByDesc('id')->first();
                    $position = 2;
                    
                    // Add "สินค้าทั้งหมด" breadcrumb
                    $breadcrumbItems[] = [
                        '@type' => 'ListItem',
                        'position' => $position++,
                        'item' => [
                            'name' => __('สินค้าทั้งหมด'),
                            '@id' => route('public.products')
                        ]
                    ];
                    ...
                    $productImages = [];
                    if ($object->image) {
                        $productImages[] = RvMedia::getImageUrl($object->image, null, false, RvMedia::getDefaultImage());
                    }
                    if ($object->images) {
                    ...
                    if ($category) {
                        ...
                        $categoryHierarchy = implode(' > ', $categoryNames);
                    }

                    // Enhanced offers with shipping details
                    $enhancedOffers = array_merge($offers, [
                        'url' => $object->url,
                        ...
                        'shippingDetails' => [
                            ...
                            'shippingRate' => [
                                '@type' => 'MonetaryAmount',
                                'value' => theme_option('shipping_fee', 100),
                                'currency' => get_application_currency()->title
                            ],
                            'shippingDestination' => [
                                '@type' => 'DefinedRegion',
                    ...
                    // Create separate schemas array (not @graph)
                    $schemas = [
                        [
                        ...
                        [
                            ...
                            'sku' => $object->sku ?: 'SKU-' . str_pad($object->getKey(), 5, '0', STR_PAD_LEFT),
                    ...
                    $schemas = apply_filters('ecommerce_product_schema', $schemas, $object);

                    $schema = json_encode($schemas, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT);
